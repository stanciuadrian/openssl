...
___
$code.=<<___ if ($SZ==8);
	test \$`1<<0`,%rax	# check for SHA512
	jnz .Lsha512ext_shortcut
___
$code.=<<___;
	and	\$`1<<8|1<<3`,%r11d	# check for BMI2+BMI1
	cmp	\$`1<<8|1<<3`,%r11d
	je	.Lavx2_shortcut
___
}
...
$code.=<<___;
	...
	# 0x510: index of last .quad entry of $K12
	vbroadcasti128 0x510+$TABLE(%rip),%ymm15
...
sub sha512op {
	...
	if (($instr eq "vsha512msg1") && ($args =~ /%xmm([0-9]|1[0-5])\s*,\s*%ymm([0-9]|1[0-5])$/)) {
		my $b1 = sprintf("0x%02x", 0x42 | ((1-int($1/8))<<5) | ((1-int($2/8))<<7) );
		my $b2 = sprintf("0x%02x", 0xc0 | ($1 & 7) | (($2 & 7)<<3)                );
		return ".byte 0xc4,".$b1.",0x7f,0xcc,".$b2;
	}
	elsif (($instr eq "vsha512msg2") && ($args =~ /%ymm([0-9]|1[0-5])\s*,\s*%ymm([0-9]|1[0-5])$/)) {
		my $b1 = sprintf("0x%02x", 0x42 | ((1-int($1/8))<<5) | ((1-int($2/8))<<7) );
		my $b2 = sprintf("0x%02x", 0xc0 | ($1 & 7) | (($2 & 7)<<3)                );
		return ".byte 0xc4,".$b1.",0x7f,0xcd,".$b2;
	}
	elsif (($instr eq "vsha512rnds2") && ($args =~ /%xmm([0-9]|1[0-5])\s*,\s*%ymm([0-9]|1[0-5])\s*,\s*%ymm([0-9]|1[0-5])$/)) {
